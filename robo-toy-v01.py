...
class Robot_Game(object):
    ...
    def move_toy(self):
        ...
        if (-1 < cx < 5) and (-1 < cy < 5):
            self.table[4-px][py] = Game.MARKER__
            self.table[4-cx][cy] = Game.MARKER_X
        else:
    ...
    def moveCommand(self):
        d = Game.CTRLS.index(self.direction)
    ...
    def rotateToyLeft(self):
        if(hasattr(self,'direction')):
            for key, value in Game.DIR_DICT.items():
    ...
    def placeTheToy(self,ctrl):
        try:
            ...
            self.prev_pos =  self.curr_pos if hasattr(self,'curr_pos') else Game.START[:]
